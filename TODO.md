...
- **Reset password** : flux PKCE corrigé dans le code (via `/auth/callback`). Côté **dashboard Supabase** (Thomas) : `https://www.arythmeethic.fr/auth/callback` doit être dans les **Redirect URLs** (✅ présent), et configurer un **SMTP custom (Brevo)** pour que les emails de reset soient réellement envoyés (le service email intégré de Supabase est trop limité).
